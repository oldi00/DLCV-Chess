...
def train(
    model, dataloader, device, epochs=35, save_model=True, save_dir=None,
    plot_loss=True, lr=1e-4, weight_decay=1e-5, empty_class_idx=12,
    empty_class_weight=0.3, val_loader=None, rank=0, use_ddp=False, patience=None
):
    if not use_ddp:
...
def evaluate_plus(
    model, dataloader, device, empty_class_idx=12, id_to_piece=DEFAULT_ID_TO_PIECE,
    compute_plots=True, title_prefix="Eval",
):
    model.eval()
    total_loss = 0.0
    all_gt = []
...
if __name__ == "__main__":
    ...
    # --- Paths (Required) ---
    parser.add_argument("--train_pkl", type=str, required=True, help="Path to the synthetic train data .pkl")
    parser.add_argument("--val_pkl", type=str, required=True, help="Path to the synthetic val data .pkl")
    parser.add_argument("--save_dir", type=str, required=True, help="Directory to save the checkpoints")
    
    # --- Hyperparameters (Optional with defaults) ---
    parser.add_argument("--epochs", type=int, default=50, help="Number of training epochs")
    parser.add_argument("--lr", type=float, default=1e-4, help="Learning rate")
    parser.add_argument("--batch_size", type=int, default=16, help="Batch size per GPU")
    parser.add_argument("--patience", type=int, default=16, help="Batch size per GPU")
